...
class Parser:
    ...
    def statement(self, stmt):
        if(stmt[0] == 'if'): 
            return self.if_statement(['if'], stmt)
        elif(stmt[0] == 'while'): 
            return self.while_loop(['while'], stmt)
        elif(len(stmt) > 1 and stmt[1] == '='):   
    ...
    def if_statement(self, out, stmt):
        cond, index2 = self.condition(stmt)
        out.append(cond)  # Appends formatted condition
        
        if(index2 == len(stmt)):  # Accepts next-line "then" statement
            self.advance()
            index2 = 0
        
        # Searches for "else"
        index3 = index2 + 1
        while(index3 < len(stmt)):    
            if(stmt[index3] == "else"): 
                out.append(self.statement(stmt[index2 : index3]))

                if(index3 == len(stmt) - 1):  # Accounts for separate-line statement after "else"
                    self.advance()
                    out.append(self.statement(stmt))
                else:   # Checks for same-line else
                    out.append(self.statement(stmt[index3 + 1:]))

                return out
            index3 += 1
                
        out.append(self.statement(stmt[index2:]))     # Adds "then" statement

        # Accepts next-line "else" statement
        if(self.lexer.get_next_is_else()):
            self.advance()
            if(len(stmt) == 1):   # Adds next-line "else" statement
                self.advance()
                out.append(self.statement(stmt))
            else: out.append(self.statement(stmt[1:]))    # Adds same-line "else" statement
    ...
    def while_loop(self, out, stmt):
        ...
        if(index2 == len(stmt)): 
            ...
            out.append(self.statement(stmt[index2:]))
        return out
...
code = '''
    if x > 1 then while (abc) > (def) do x = x + 1 else while (efg) > hij do y = y + 1
    '''
test_parser(code)